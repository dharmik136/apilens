...
ORDER BY (project_id, app_id, metric_name, timestamp)
TTL toDateTime(timestamp) + INTERVAL 90 DAY
SETTINGS index_granularity = 8192;

-- Bloom filter indexes for efficient filtering
ALTER TABLE metrics ADD INDEX idx_metric_project_id project_id TYPE bloom_filter(0.01) GRANULARITY 1;
ALTER TABLE metrics ADD INDEX idx_metric_app_id app_id TYPE bloom_filter(0.01) GRANULARITY 1;
...
ENGINE = MergeTree()
PARTITION BY toYYYYMM(timestamp)
ORDER BY (project_id, app_id, timestamp)
TTL toDateTime(timestamp) + INTERVAL 30 DAY
SETTINGS index_granularity = 8192;

-- Bloom filter indexes
ALTER TABLE logs ADD INDEX idx_logs_project_id project_id TYPE bloom_filter(0.01) GRANULARITY 1;
ALTER TABLE logs ADD INDEX idx_logs_app_id app_id TYPE bloom_filter(0.01) GRANULARITY 1;
...
-- Indexes
ALTER TABLE traces ADD INDEX idx_traces_project_id project_id TYPE bloom_filter(0.01) GRANULARITY 1;
ALTER TABLE traces ADD INDEX idx_traces_app_id app_id TYPE bloom_filter(0.01) GRANULARITY 1;
...
ENGINE = MergeTree()
PARTITION BY toYYYYMM(timestamp)
ORDER BY (project_id, app_id, event_type, timestamp)
TTL toDateTime(timestamp) + INTERVAL 90 DAY
SETTINGS index_granularity = 8192;

-- Indexes
ALTER TABLE events ADD INDEX idx_events_project_id project_id TYPE bloom_filter(0.01) GRANULARITY 1;
ALTER TABLE events ADD INDEX idx_events_app_id app_id TYPE bloom_filter(0.01) GRANULARITY 1;
